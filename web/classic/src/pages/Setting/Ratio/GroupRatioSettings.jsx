    ...
    <Form key='form-visual' values={inputs} style={{ marginBottom: 15 }}>
      <Form.Section text={t('分组管理')}>
        ...
        <GroupTable
          ...
          conversationLogDisabledGroups={inputs.ConversationLogDisabledGroups}
      ...
      <Form.Section text={t('分组JSON设置')}>
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              label={t('不记录对话的分组')}
              placeholder={t('为一个 JSON 字符串数组，例如：["vip"]')}
              extraText={t(
                '列在这里的分组不会写入对话历史，也不会出现在 QA JSON 导出中。',
              )}
              field={'ConversationLogDisabledGroups'}
              ...
              onChange={(value) =>
                setInputs((prev) => ({
                  ...prev,
                  ConversationLogDisabledGroups: value,
                }))
              }
            />
          </Col>
